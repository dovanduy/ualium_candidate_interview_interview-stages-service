...
{
    public class DeclineOfferCommandHandler : IConsumer<IDeclineOfferCommandRequest>
    {
        private static readonly Logger Logger = LogManager.GetCurrentClassLogger();

        public async Task Consume(ConsumeContext<IDeclineOfferCommandRequest> context)
        {
                ...
                */
                try
                {
                    using (var connection = new SqlConnection(InterviewStagesServiceDbContext.Connectionstring.GetConnection))
                    {
                        ...
                        context.Respond(new DeclineOfferCommandResponse());
                    }
                }
                catch (Exception ex)
                {
                    const string message = "Could not insert Candidate Declined Offer.";

                    Logger.Error($"{message} {ex.Message}");

                    var response = new DeclineOfferCommandResponse { Errors = new List<Error>()};
